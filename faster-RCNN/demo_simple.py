...
if __name__ == '__main__':
  ...
  input_dir = args.load_dir + "/" + args.net + "/pascal_voc"
  if not os.path.exists(input_dir):
    raise Exception('There is no input directory for loading network from ' + input_dir)
  load_name = os.path.join(input_dir,
    'faster_rcnn_{}_{}_{}.pth'.format(args.checksession, args.checkepoch, args.checkpoint))

  MI3_classes = np.asarray(['__background__',
...
#  pascal_classes=np.asarray(imdb.classes)
  ...
  if args.net == 'vgg16':
    fasterRCNN = vgg16(MI3_classes, pretrained=True, class_agnostic=args.class_agnostic)
  elif args.net == 'res101':
    fasterRCNN = resnet(MI3_classes, 101, pretrained=True, class_agnostic=args.class_agnostic)
  elif args.net == 'res50':
    fasterRCNN = resnet(MI3_classes, 50, pretrained=False, class_agnostic=args.class_agnostic)
  elif args.net == 'res152':
    fasterRCNN = resnet(MI3_classes, 152, pretrained=False, class_agnostic=args.class_agnostic)
  else:
  ...
  while (num_images >= 0):
          ...
          # Optionally normalize targets by a precomputed mean and stdev
            if args.class_agnostic:
                ...
                if args.cuda > 0:
                    ...
                    box_deltas = box_deltas.view(-1, 4) * torch.FloatTensor(cfg.TRAIN.BBOX_NORMALIZE_STDS) \
                               + torch.FloatTensor(cfg.TRAIN.BBOX_NORMALIZE_MEANS)
                box_deltas = box_deltas.view(1, -1, 4 * len(MI3_classes))
      ...
      for j in xrange(1, len(MI3_classes)):
          ...
          if inds.numel() > 0:
            ...
            for i in range(np.minimum(10, dets.shape[0])):
                ...
                th=0.5
                class_name = MI3_classes[j].strip().replace(" ", "_")
                if score > th:
                    fo2.write(
                            class_name+' '+str(score)+' '+str(bbox[0])+' '+str(bbox[1])+' '+str(bbox[2])+' '+str(bbox[3])+'\n'
                            )
            if vis:
              im2show = vis_detections(im2show, MI3_classes[j], cls_dets.cpu().numpy(), 0.5)
      fo2.close()
      misc_toc = time.time()
