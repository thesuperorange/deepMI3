...
folder = 'C:/Users/superorange/Google 雲端硬碟/NCHC_backup/SharedDrive/MI3_detect_results/'
...
if __name__ == '__main__':
    ...
    for method in method_list:
        for dataset in dataset_list:
            ...
            if output_type==1:
                output_folder = os.path.join(folder,method, dataset +folder_concat_str+ '_merge')
                if not os.path.exists(output_folder):
                    os.makedirs(output_folder)

            inputFile = os.path.join(folder, method,dataset+folder_concat_str+'.txt')
